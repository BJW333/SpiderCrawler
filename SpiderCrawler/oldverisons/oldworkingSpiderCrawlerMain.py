...
def display_exploit_details(cve_id):
    ...
    if cve_details is None:
        ...
        for cve_id in cve_details: #new else statement remove if issue and unindent the below text
    #results_text.insert(tk.INSERT, "\n")
            results_text.insert(tk.INSERT, "\n---------------------------------------------------------------------\n")
            ...
            results_text.insert(tk.INSERT, "\n---------------------------------------------------------------------\n")
            results_text.insert(tk.INSERT, (f"\nSearching GitHub and Google for potential exploits for {cve_details.get('id')}...\n")) #cve_id is the old variable
            search_url = f"https://github.com/search?q={cve_details.get('id')}+exploit&type=Repositories" #cve_id is the old variable
            results_text.insert(tk.INSERT, (f"\nCheck out GitHub search results at: {search_url}\n"))
            query = f"https://www.google.com/search?q={cve_details.get('id')}+exploit" #cve_id is the old variable
...
def get_ip_website():
    ...
    try:
        ...
        results_text.insert(tk.INSERT, ip_address + "\n")
    except socket.gaierror as e:
        results_text.insert(tk.INSERT, f"Error getting IP for {domain}: {e}\n")
