...
#new method below which checks cve patterns
def parse_nmap_vuln_output(nmap_output):
    open_ports = []
    nmap_vulnerabilities = {}
    cve_pattern = re.compile(r"CVE-\d{4}-\d{4,7}")  # Adjusted pattern to match typical CVE IDs
    for line in nmap_output.split('\n'):
        if '/tcp' in line and 'open' in line:
            port = line.split('/')[0].strip()
            open_ports.append(port)
...
#may have to remove below method it might not be being used for anything it returns null stuff
def search_exploitdb(vulns):
    ...
    print(exploits, "bstest exploits") #bugtesting this method
    return exploits
